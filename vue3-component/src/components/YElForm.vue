<template>
  <div class="search_input">
    <el-form :size="size"
      ...
      <el-form-item v-for="item in searchForm"
        ...
        <el-select v-if="item.type === 'Select'"
          ...
          <el-option v-for="op in item.options"
                     :label="op.label"
                     :value="op.value"
                     :key="op.value">{{ op.label }}</el-option>
        </el-select>
        ...
        <el-radio-group v-if="item.type === 'Radio'"
          ...
          <el-radio v-for="ra in item.radios"
                    :label="ra.value"
                    :key="ra.value">{{ ra.label }}</el-radio>
        </el-radio-group>
        ...
        <el-radio-group v-if="item.type === 'RadioButton'"
          ...
          <el-radio-button v-for="ra in item.radios"
                           :label="ra.value"
                           :key="ra.value">{{ ra.label }}</el-radio-button>
        </el-radio-group>
      ...
      <!-- button 操作按钮 -->
      <el-form-item class="search_input_button"
                    v-if="isHandle">
        <el-button v-for="item in searchHandle"
                   :key="item.label"
                   :type="item.type"
                   @click="item.handle()"
                   v-if="!item.hide">
...
//  链接 https://www.jianshu.com/p/93e767b6840c
</script>